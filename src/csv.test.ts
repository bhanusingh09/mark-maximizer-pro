...
test("imports the documented marksheet columns", () => {
  assert.deepEqual(parseMarksheetCsv("Roll number,Student name,Marks\n21CS001,Aditi Rao,18"), [
...
test("reports missing headers and malformed quoted fields", () => {
  assert.throws(() => parseMarksheetCsv("Roll number,Marks\n21CS004,17"), /missing Student name/);
  assert.throws(
