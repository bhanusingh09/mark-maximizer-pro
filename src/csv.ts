...
export function parseMarksheetCsv(text: string): CsvMarksheetRow[] {
  ...
  if (missingHeaders.length > 0) {
    throw new Error(
      `The CSV is missing ${missingHeaders.join(", ")}. Use columns named Roll number, Student name, and Marks.`,
    );
  }
  ...
  if (records.length > MAX_CSV_RECORDS) {
    throw new Error(
      `The CSV contains more than ${MAX_CSV_RECORDS.toLocaleString()} student records.`,
    );
  }
  ...
  return records.map((cells) => {
    ...
    return {
      ...
      score:
        typeof numericScore === "number" && Number.isFinite(numericScore) ? numericScore : null,
    };
  });
}
